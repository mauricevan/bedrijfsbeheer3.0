...
**Important:** Test files that are created during test execution (such as temporary files, mock data files, or generated test artifacts) must be removed after test completion to maintain a clean codebase and prevent unnecessary files from being committed to version control.

**Best Practices:**
- ✅ Use `beforeEach`/`afterEach` hooks to clean up temporary files
- ✅ Use `afterAll` hooks for cleanup after all tests complete
- ✅ Ensure test cleanup runs even when tests fail (use `try/finally` blocks)
- ✅ Add temporary test files to `.gitignore` if they might be created
- ✅ Document cleanup procedures in test setup files
...
describe('File Processing', () => {
  const testFilePath = join(__dirname, 'temp-test-file.txt');

  beforeEach(async () => {
    // Create test file before each test
    await writeFile(testFilePath, 'test data');
  });
  ...
  it('processes file correctly', async () => {
    // Test implementation
    // Test file will be cleaned up automatically
  });
});
```
...
- [ ] Temporary test files removed after test execution
- [ ] No test artifacts committed to version control
```

...
export const QuoteForm: React.FC<QuoteFormProps> = ({ quote, onSubmit, onCancel, isLoading }) => {
  ...
  useEffect(() => {
    if (quote) {
      setFormData({
        ...
        scheduledDate: quote.scheduledDate ? new Date(quote.scheduledDate).toISOString().split('T')[0] : '',
        notes: quote.notes || '',
      });
      setLineItems(quote.items || []);
      setLaborItems(quote.labor || []);
    }
  }, [quote]);
